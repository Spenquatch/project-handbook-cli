...
def _write_passing_sprint_gate_task(ph_root: Path) -> None:
    ...
    (task_dir / "steps.md").write_text("# Steps\n\n1) Run the gate checks\n", encoding="utf-8")
    (task_dir / "commands.md").write_text(
        "\n".join(
            [
                "# Commands",
                "",
                "Evidence path: status/evidence/TASK-000/pre-exec-lint.txt",
                "",
            ]
        ),
        encoding="utf-8",
    )
    (task_dir / "checklist.md").write_text("# Checklist\n\n- [x] Gate complete\n", encoding="utf-8")
    (task_dir / "validation.md").write_text(
        "\n".join(
            [
                "# Validation",
...
def test_pre_exec_lint_stdout_matches_make_pre_exec_lint_preamble(tmp_path: Path) -> None:
    ...
    _write_passing_sprint_gate_task(tmp_path)
