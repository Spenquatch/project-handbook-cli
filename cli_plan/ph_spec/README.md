...
This folder contains the directory-by-directory spec for the v1 Project Handbook **repo-root layout** (content under `PH_ROOT/**` + internals under `PH_ROOT/.project-handbook/**`), plus example artifacts.
...
- As a result, **internal relative links may not resolve** from within the `examples/` location (because the examples are nested deeper than real repo-root content paths).
