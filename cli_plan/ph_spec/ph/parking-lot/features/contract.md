---
title: PH Spec Contract — parking-lot/features/
type: contract
tags: [ph, spec]
---
...
## Directory Purpose
- Path (handbook instance): `PH_ROOT/parking-lot/features/`
- Summary: Active feature parking-lot items (not yet promoted into roadmap/backlog), organized as one directory per item.
...
## Creation
- Created/updated by:
  - `ph parking add --type features --title "..." ...` (creates a new item directory + `README.md` and updates `parking-lot/index.json`).
  - `pnpm make -- parking-add type=features title="..." ...` (creates a new item directory + `README.md` and updates `parking-lot/index.json`).
- Non-destructive:
...
## Examples Mapping
- No active `features` fixtures are currently provided in `examples/`; items are created by `ph parking add --type features ...`.
