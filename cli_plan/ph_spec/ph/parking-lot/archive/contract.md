...
- Created/updated by:
  ...
  - Parking-lot items are archived by moving an existing `ph/parking-lot/<category>/<ITEM-ID>/` directory into `ph/parking-lot/archive/<category>/<ITEM-ID>/` (manual or future CLI command).
- Non-destructive:
...
- `ph validate` SHOULD enforce:
  ...
  - archived items are excluded from `ph/parking-lot/index.json` (catalog contains only active parking-lot items)
